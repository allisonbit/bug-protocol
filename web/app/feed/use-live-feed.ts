 ...
 *
 * Realtime must be enabled on `public.events` (swarm.sql adds it to the
 ...
 */
export function useLiveFeed(seed: SwarmEvent[], cap = 100): { events: SwarmEvent[]; live: boolean } {
  const [events, setEvents] = useState<SwarmEvent[]>(seed);
  ...
  useEffect(() => {
    ...
    const channel = sb
      ...
      .on(
        ...
        (payload: RealtimePostgresChangesPayload<SwarmEvent>) => {
          const row = payload.new as SwarmEvent;
