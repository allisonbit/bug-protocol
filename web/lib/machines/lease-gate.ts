 ...
 * it stops happening.
 *
 * WHAT IT DOES, IN ORDER. It reads the live leases for one machine and one scope,
 * takes the EARLIEST-expiring one (pickLease, the conservative choice), asks the pure
 * rule whether that lease authorizes this act right now, and only then spends one
 * actuation from the ceiling. If any of those refuse, nothing is written anywhere and
 * the caller gets a code and a sentence a person can read.
 *
...
export async function takeActuationAuthority(
  ...
  const lease = pickLease((data as LeaseRow[] | null) ?? [], input.scope);
