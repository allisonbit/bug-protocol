...
/** Collapse a run of the same letter to one, the same transform tokens() does,
 * so the lookup keys below match what a shattered token reduces to. */
const collapse = (w: string) => w.replace(/(.)\1+/g, "$1");

/**
 * The number words, keyed by their COLLAPSED form. The shattering doubles
 * letters ("OoO"), but real words carry doubles too ("three"), so the map must
 * be built from the collapsed spelling or "three" would only ever be found as
 * "thre". Keying collapsed handles both at once: "twenntyy" and "twenty" both
 * land on "twenty", and "three" and "thre" both land on "thre".
 */
