...
export type ListingKind = "mcp-registry" | "clawhub" | "agent-skills-index" | "discovery-documents";
...
// ---------------------------------------------------------------------------
// This domain's own discovery documents
// ---------------------------------------------------------------------------

/**
 * The documents a runtime finds by guessing a bare domain, and nothing else.
 *
 * Every path here is one a client tries with no directory, no listing and no human
 * in the loop, which is what makes them worth watching in a way that a page linked
 * from the footer is not: nobody is looking at them when they break.
 */
...
/** Bounded, so one document full of links cannot turn an hourly beat into a crawl. */
const MAX_ADVERTISED_PROBES = 40;

/**
 * Every string in a PARSED document that names an address on this site.
 *
 * WHY THIS IS NOT RUN OVER THE RAW TEXT, WHICH IS THE OBVIOUS WAY TO WRITE IT.
 * The prose documents — `skill.md`, `agents.md`, `llms.txt` — name addresses in
 * worked examples, and a worked example is deliberately generic. Measured against
 * this deployment: a text pass finds 64 on-site URLs and five of them 404, every one
 * of them a placeholder (`/agents/YOUR-HANDLE/document`, `/v1/outputs/OUTPUT_ID/review`,
 * `?post=1234`). A check that reported those as faults would have this platform
 * telling readers its own contract is broken, once an hour, forever. A block of
 * heuristics to recognise placeholders would be a guess standing in for a fact, so
 * the pass reads JSON values: a manifest naming `api.url` or `logo_url` is a machine
 * promise, and a sentence illustrating a call is not.
 *
 * A relative path is resolved against this site, because that is exactly what the
 * client reading the document would do with it.
 */
function advertisedUrls(value: unknown, out = new Set<string>()): Set<string> {
  if (typeof value === "string") {
    if (/^https?:\/\//.test(value)) {
      if (value.includes(apexDomain()) || value.startsWith(SITE_URL)) out.add(value);
    } else if (/^\/[a-zA-Z0-9._~!$&'()*+,;=:@%/-]*$/.test(value) && value.length > 1) {
      out.add(`${SITE_URL}${value}`);
    }
    return out;
  }
  if (Array.isArray(value)) {
    for (const v of value) advertisedUrls(v, out);
    return out;
  }
  if (value && typeof value === "object") {
    for (const v of Object.values(value as Record<string, unknown>)) advertisedUrls(v, out);
  }
  return out;
}

/**
 * This domain's own promise, read the way an arriving client reads it.
 *
 * WHAT THIS WATCHES. An agent pointed at this domain discovers it by guessing a
 * path. Each of those paths is a promise, and a promise fails in three ways no other
 * check here covers:
 *
 *   1. The document stops answering — a rewrite dropped from `next.config.ts`, a
 *      route that began throwing. Nothing else in this repository notices, because
 *      nothing else requests these paths.
 *   2. It answers with something that does not parse. A strict client treats
 *      malformed JSON as an absent document, so the failure arrives as a domain that
 *      appears to have no manifest rather than as an error.
 *   3. It names a URL that no longer exists. This is the quiet one: the document
 *      reads as healthy, the reader believes the door is there, and the fault only
 *      shows up later as an arrival that did not happen.
 *
 * WHAT IT DELIBERATELY DOES NOT CALL A FAILURE. A 401, 403 or 405 from an advertised
 * address is an ANSWER: the door is there and wants a credential or another verb.
 * These documents legitimately advertise endpoints that require an agent key, so
 * treating those as broken would make the check wrong about most of the contract.
 *
 * WHY IT CANNOT REPAIR ANYTHING. These documents are generated by the code that is
 * running, so a drifted one is a defect a deploy fixes, not a storefront that went
 * dark. `checkListings` says so rather than claiming a repair it did not perform.
 */
async function checkDiscoveryDocuments(): Promise<Omit<ListingResult, "previous" | "repaired" | "repairDetail" | "checkedIso">> {
  const base = {
    listing: "discovery-documents",
    kind: "discovery-documents" as const,
  ...
  for (const path of DISCOVERY_DOCUMENTS) {
    let res: Response;
    try {
      ...
      problems.push(`${path} could not be fetched (${e instanceof Error ? e.message : String(e)})`);
      continue;
    }
    const body = await res.text();
    if (!res.ok) {
      problems.push(`${path} returns ${res.status}`);
      continue;
    }
    if (body.trim().length === 0) {
      problems.push(`${path} answers ${res.status} with an empty body`);
      continue;
    }
    read += 1;
    if (path.endsWith(".json")) {
      try {
        advertisedUrls(JSON.parse(body), advertised);
      } catch {
        problems.push(`${path} is served as JSON and does not parse`);
      }
    }
  }

  const links = [...advertised].slice(0, MAX_ADVERTISED_PROBES);
  let probed = 0;
  for (const url of links) {
    ...
    try {
      ...
      problems.push(`${url} could not be reached`);
      continue;
    }
    probed += 1;
    const answered = status < 400 || status === 401 || status === 403 || status === 405;
    if (!answered) problems.push(`${url} answers ${status}`);
  }
  ...
  if (problems.length > 0) {
    return {
      ...base,
      state: "missing",
      // The first few, not all of them: a detail field that grows without bound is
      // one nobody reads, and the count says how much is behind it.
      detail: `${problems.length} problem(s): ${problems.slice(0, 4).join("; ")}`,
      version: null,
    };
  }
  ...
  return {
    ...base,
    state: "present",
    detail: `${read} document(s) answered and parsed, ${probed} address(es) they advertise answered`,
    version: null,
  };
}
...
export async function checkListings(sb: SupabaseClient, options: { repair?: boolean } = {}): Promise<ListingsReport> {
  ...
  const checks: Finding[] = [
    ...
    await checkDiscoveryDocuments().catch((e) => ({
      listing: "discovery-documents",
      kind: "discovery-documents" as ListingKind,
      title: "Discovery documents",
      url: `${SITE_URL}/.well-known/mcp.json`,
      state: "error" as ListingState,
      detail: e instanceof Error ? e.message : String(e),
    ...
    if (state === "missing" && repairEnabled) {
      try {
        if (found.listing === "mcp-registry") {
          ...
          // Nothing can repair these from inside the deployment, and the reason is
          // the same for both: the document is generated by the code that is
          // running here, so one that has drifted is a defect a deploy fixes rather
          // than a storefront that went dark. Saying "repaired" would be a lie about
          // a check nobody performed.
          repairDetail =
            found.listing === "agent-skills-index"
              ? "not repairable from here: the index is generated from this deployment's own code, so a mismatch is a defect a deploy fixes"
              : "not repairable from here: these documents are generated by this deployment, so one that has drifted is a defect a deploy fixes";
        }
      } catch (e) {
...
export async function readListingHealth(sb: SupabaseClient): Promise<ListingResult[]> {
  ...
  const ORDER: ListingKind[] = ["mcp-registry", "clawhub", "agent-skills-index", "discovery-documents"];
