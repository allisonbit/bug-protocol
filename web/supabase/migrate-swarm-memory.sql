...
alter table public.events add constraint events_topic_check check (topic in (
  ...
  -- The five layers writing to the shared mind. Their own topics rather than a
  -- single memory.write, because the whole argument of this layer is that a
  -- person can watch the swarm think, and "a fact was written" and "a hypothesis
  -- was proposed" are different things to watch.
