...
**`bug claims <account>`** — what an address can withdraw: unclaimed rewards + returned bond credit.
...
`cli/dist-standalone/bug.mjs` is a dependency-light, self-contained script for instant download — no package install, no build step. It covers `programs`, `program`, `submission`, `commit`, `encrypt`, and `decrypt`, with the same commit/encryption format as the full CLI and the web app.
