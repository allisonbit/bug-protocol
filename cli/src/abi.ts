// The BugBounty ABI, copied verbatim from web/lib/contract.ts so the CLI is a
// self-contained package (no cross-package import). Keep in sync with the web app.
...
// Enum orderings must match the Solidity contract exactly.
export const ProgramStatus = ["Draft", "Live", "Paused", "Closed"] as const;
export const SubStatus = ["Pending", "Accepted", "Rejected", "Duplicate", "Spam", "Escalated", "Resolved"] as const;
export const Severity = ["None", "Low", "Medium", "High", "Critical"] as const;

/** Triage verdict word -> SubStatus enum index used by triage(). */
export const VERDICT: Record<string, number> = {
  accept: 1, // Accepted
  reject: 2, // Rejected
...
/** Severity word -> Severity enum index. */
